...
Ship::Ship() : coordinates{ 0.0, 0.0, 0.0 },
               currentPlanet("NULL", "Unknown", 0.0, Biome::Barren, 0, { 0,0,0 }), //Placeholder planet
               docked(false),
               hovering(true),
               radar(3)    //Default scan range
{}

// Coord setter and getter
void Ship::setCoordinates(const array<double, 3>& coords) { coordinates = coords; }
...
void Ship::getNearbyPlanet(Game& game)
{
    ...
    for (const auto& planet : allPlanets) 
    {
        ...
        // The Euclidean distance formula to find how the distance from the planet to the ship in 3D  
        double dist = sqrt(dx * dx + dy * dy + dz * dz);

        planetDistances.emplace_back(dist, planet); // Stores each planet together with calculated distance and its name
    }
    ...
    for (int i = 0; i < static_cast<int>(lastScannedPlanets.size()); ++i)
        display << "(" << (i + 1) << ") " << lastScannedPlanets[i].quickRow(fuelPerAU) << "\n";

    display << "\n(Type 'travel to 1', 'travel to 2', or 'travel to 3' to visit a planet.)";
