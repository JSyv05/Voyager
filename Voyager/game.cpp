...
Game::Game()
    : text_output(""), error_output(""), art_output(""),
      clear_screen("\033[2J\033[1;1H"), start_game(false) {

    /*
    - clear_screen uses an ANSI escape code, a highly extensible feature that
    can allow you to format text format for ansi escape characters:
    <escape-character>[<parameter><command>
    - \033 represents the escape character. \x1b and \e are also viable options
    for their own reasons
    - in this case there are two ANSI escape codes being used
    - \033 initializes the escape character, [ is the Control Sequence Intoducer
    and allows us to pass certain parameters
    - J is the command to erase in display, 2 is the parameter that erases the
    entire screen. 1 would erase everything behind the cursor
    - The same logic applies to the second ANSI escape character \033[1;1H
    - H is the command for the cursor
    - 1;1 is the position the cursor will return to, in this case, the beginning
    of the file
    */
...
void Game::clearScreen() const { cout << clear_screen; }
...
void Game::setInput() {
    ...
    stringstream untokenizedInput(
        input);   // Convert the input to a stringstream to tokenize input
    string token; // Temp variable that will store the individual tokens of the
                  // input

    while (untokenizedInput >>
           token) { // While loop that pushes each token to the end of the list
        ...
        tokens.push_back(
            token); // Appends a the lowercased token to the end of the list
...
vector<string>* Game::getInput() {
    return &tokens;
} // Returns a pointer to the vector of tokens, which will be used to select
  // individual tokens
